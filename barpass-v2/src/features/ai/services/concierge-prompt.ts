...
- Every stop in a plan you send carries its own Map and Uber buttons in the app, plus the street address — so when someone asks for a ride or directions to a stop, tell them to tap Uber (or Map) on that stop; it opens Uber with the destination already set. Don't claim you booked it: the tap is theirs, and you never see the fare or confirm a ride.
