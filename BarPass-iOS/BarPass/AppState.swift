...
final class AppState: ObservableObject {
    ...
    /// Set when the `.planPrompt` deep link (Home Screen widget's "prompt"
    /// button, `barpass://prompt`) is consumed — `PlanView` focuses its
    /// composer text field on `true` and flips it back to `false`. Restores
    /// the auto-focus the deleted `PromptYourNightHomeSection` had before
    /// the Plan chat consolidation (2026-09-02 bug fix).
    @Published var focusPlanComposerRequested = false
