...
/// Real entitlement check via StoreKit 2 — the source of truth for whether
/// Plan chat should treat this user as Premium (longer plans, no daily cap,
/// remembered preferences; see `PlanUsageService` and `PlanPreferencesService`).
///
...
/// `com.sebastian.barpass` — that's an App Store Connect setup step, not
/// something fixable from code. `productID` below is a placeholder; once a
/// real product exists with that identifier (or this constant is updated to
/// match whatever identifier is actually created), this starts working with
/// no other code changes — `Product.products(for:)` and
